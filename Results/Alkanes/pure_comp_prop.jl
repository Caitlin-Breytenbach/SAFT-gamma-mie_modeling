...
for i in 1:N 
    ...
    Cp_exp = CSV.read(data_dir * "$(name)_Cp.csv", DataFrame; header=3)
...
#     save("$(name)_rhol_vs_pressure.png", fig_rhol; px_per_unit=3)
    ...
    Cp_curves = Dict(
        "Papaioannou" => Cp_plot(model_lit,    Cp_exp),
        "fitted"      => Cp_plot(model_fitted, Cp_exp),
        # "weighted"      => Cp_plot(model_weighted, Cp_exp),
    )

    fig_Cp = plot_Cp(Cp_curves;
        exp_T   = Cp_exp.T,
        exp_p   = Cp_exp.p,
        exp_Cp = Cp_exp.out_Cp,
    )
